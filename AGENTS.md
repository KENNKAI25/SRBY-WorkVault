...
custom-width: 42
---
# AGENTS.md — Instruksi Sistem Global untuk Kenny

File ini wajib dibaca Codex setiap kali bekerja di folder mana pun di bawah `C:\KEN Work Vault`. Instruksi di sini menimpa default behavior.

*Last updated: 10 Mei 2026 — Bagian B di-spin-off ke `About/Brand Context.md`. AGENTS.md sekarang fokus ke aturan sistem.*
...
Sebelum mulai task apa pun, Codex **wajib baca 5 file di folder `01 - About/`** (root folder `KEN Work Vault`):
...
- **❌ Airtable MCP — JANGAN dipakai untuk task Saramonic atau BOYA.** Abaikan koneksi Airtable MCP server sepenuhnya saat ngerjain task apa pun yang menyangkut Saramonic atau BOYA. Jangan query, baca, atau tulis ke Airtable untuk kebutuhan dua brand ini — sumber kebenaran tetap Todoist (data marketing activity) + file lokal di `KEN Work Vault` (konteks brand). Airtable hanya boleh dipakai kalau Kenny minta eksplisit untuk task non-Saramonic/BOYA.
...
- Kalau ada konflik info antara `01 - About/Brand Context.md` dan output Codex → ikutin `Brand Context.md`. Kalau data di `01 - About/Brand Context.md` masih `[isi di sini]` atau `TBD` → tanya Kenny dulu, jangan ngarang.
- **Folder lain mungkin punya `AGENTS.md` lokal sendiri** — contoh: `03 - Marketing Repo/AGENTS.md` untuk task marketing, `04 - Sales Data/AGENTS.md` untuk task pricing & sales. Kalau task masuk ke folder yang punya local AGENTS.md, baca file itu juga sebagai instruksi tambahan (di atas instruksi global ini, tapi tidak menimpa Hard Rules §5).

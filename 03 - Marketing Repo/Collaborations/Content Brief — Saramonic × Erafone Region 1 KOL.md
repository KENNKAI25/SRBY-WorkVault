# Content Brief — Saramonic × Erafone Region 1
Program KOL Support | Saramonic Indonesia

**Scope:** 9 toko · 3 KOL · Produk Air & Air SE

---

## BRIEF — @dizzlipin
**Type:** Online KOL Support
**Platform:** Instagram (primary) / TikTok
**Product:** Saramonic Air SE
**Format:** Unboxing & Review

### Deliverable
- 1 video: unboxing + review Saramonic Air SE
- Durasi: 90 - 180 detik
- Mention Erafone sebagai tempat beli resmi di akhir video

### Key Message (wajib masuk)
- Air SE plug & play — langsung connect ke HP/kamera tanpa setup ribet
- Wireless 2.4 GHz, form factor compact
- Harga: Air SE-01 Rp 780.000 (single) / Air SE-02 Rp 1.099.000 (dual)
- Bisa dibeli di Erafone — toko fisik & online

### Struktur Video
1. Buka packaging — tunjukkan isi box (TX, RX, charging case)
2. Pasang ke HP — demonstrasikan kemudahan setup
3. Demo audio: bandingkan suara Air SE vs mic bawaan HP
4. Closing verbal: "Bisa di-grab di Erafone ya"

### Visual Notes
- Tampilkan ukuran produk di tangan (scale reference)
- Lighting cukup — detail produk harus keliatan
- Tunjukkan indikator pairing di layar HP jika ada

### Mandatory
- Hashtag: `#Saramonic` `#SaramonicID` `#Erafone` `#saramonicairse`
- Tag: `@saramonic_id` & IG Erafone Region 1
- Mention Erafone di akhir konten — verbal atau text overlay

### Jangan
- Klaim spec di luar yang tertera di produk
- Sebut nama brand kompetitor secara langsung

**Tanggal tayang:** [TBD]

---

## BRIEF — @akkangheyhe
**Type:** Offline KOL Support — Store Visit
**Platform:** Instagram / TikTok
**Product:** Saramonic Air (Air-01 atau Air-02)
**Format:** Visit toko organik — natural, tidak terasa seperti iklan

### Deliverable
- 1 video: store visit Erafone Region 1
- Durasi: 90 - 180 detik
- Toko: pilih salah satu dari 9 lokasi Erafone Region 1 (lihat daftar di atas)

### Concept
Creator datang ke toko sebagai pembeli biasa. Sales counter Saramonic menjelaskan produk secara natural di depan kamera. Tidak ada script kaku — konten harus terasa organik.

### Key Spec untuk Sales Demo
- 90 dB SnR — tertinggi di kelasnya untuk wireless mic mini
- Satu-satunya wireless mic mini dengan opsi lavalier
- Ada layar monitoring pada receiver
- Harga: Air-01 Rp 1.999.000 / Air-02 Rp 2.499.000

### Struktur Video
1. Creator masuk toko — approach counter Saramonic
2. Sales demo produk natural — creator aktif tanya-jawab
3. Closing: creator putuskan beli — tampilkan di kasir / bag dengan produk, sebutkan juga dimana saja customer bisa nyoba & beli Saramonic (Region 1)

### Mandatory
- Hashtag: `#Saramonic` `#SaramonicID` `#Erafone` `#saramonicair`
- Tag: `@saramonic_id` & tag toko yang di visit

### Jangan
- Script yang kedengeran dihafal
- Sebut nama brand kompetitor
- Opening overlay "AD" / "Collab" yang terlalu obvious

**Tanggal tayang:** [TBD]

---

## BRIEF — @rizalhamzahnasution
**Type:** Offline KOL Support — Store Visit
**Platform:** Instagram / TikTok
**Product:** Saramonic Air SE (Air SE-01 atau Air SE-02)
**Format:** Visit toko organik — natural, tidak terasa seperti iklan

### Deliverable
- 1 video: store visit Erafone Region 1
- Durasi: 90 - 180 detik
- Toko: pilih salah satu dari 9 lokasi Erafone Region 1 (lihat daftar di atas)

### Concept
Creator datang ke toko sebagai pembeli biasa. Sales counter Saramonic menjelaskan produk secara natural di depan kamera. Tidak ada script kaku — konten harus terasa organik.

### Key Spec untuk Sales Demo
- Plug & play — connect langsung ke HP/kamera
- Dapat digunakan sebagai mic todong
- Terdapat fitur AI Noise cancellation yang dapat membedakan suara manusia & noise
- Entry-level, harga terjangkau untuk creator pemula
- Harga: Air SE-01 Rp 780.000 / Air SE-02 Rp 1.099.000

### Struktur Video
1. Creator masuk toko — approach counter Saramonic
2. Sales demo produk natural — creator aktif tanya-jawab
3. Closing: creator putuskan beli — tampilkan di kasir / bag dengan produk, sebutkan juga dimana saja customer bisa nyoba & beli Saramonic (Region 1)

### Mandatory
- Hashtag: `#Saramonic` `#SaramonicID` `#Erafone` `#saramonicairse`
- Tag: `@saramonic_id` & tag toko yang di visit

### Jangan
- Script yang kedengeran dihafal
- Sebut nama brand kompetitor
- Opening overlay "AD" / "Collab" yang terlalu obvious

**Tanggal tayang:** [TBD]

---

## Action Plan (setelah meeting)
1. **Setup jadwal visit offline toko** — koordinasi jadwal kunjungan KOL offline (@akkangheyhe, @rizalhamzahnasution) ke toko Erafone Region 1.
2. **Setup collab post** — finalisasi format konten, timeline posting, dan approval brief bersama KOL online (@prisciliatanbayong).
3. **Setup brief special request** — akomodasi brief khusus / special request dari Erajaya.
